...
def critic_loss_wgan_gp(disc_params, gen_params, real_batch, rng, 
                        ...
                        alpha_acf=0, alpha_leverage=0, alpha_emd=1.0):
    """
    Composite WGAN-GP critic loss = alpha_emd * (mean(D(fake)) - mean(D(real)))
                                    + alpha_acf * ACF_score + alpha_leverage * leverage_score
    ...
    # 6) Composite terms (same as generator)
    # Inverse transform fake_out for metrics
    generated_data_transformed = inverse_transform_jax(fake_out.squeeze(-1), transform_params)
    if alpha_acf > 0:
        acf_score = ACF_score_jax(generated_data_transformed, benchmark_lags, benchmarks)
        acf_loss = acf_score * alpha_acf
    else:
        acf_loss = 0.0
    if alpha_leverage > 0:
        leverage_score = leverage_score_jax(generated_data_transformed, benchmark_lags, benchmarks)
        leverage_loss = leverage_score * alpha_leverage
    else:
        leverage_loss = 0.0
    # 7) Total composite loss
    composite_loss = alpha_emd * wgan_loss + acf_loss + leverage_loss + gp_term
...
@partial(jax.jit, static_argnames=("critic_model", "disc_optimizer", "noise_dim", "n_layers", "chopsize", "bond_dim", "gradient_penalty_weight", "alpha_acf", "alpha_leverage", "alpha_emd"))
def train_critic_jax(disc_params, disc_opt_state, gen_params, real_batch, rng,
                     ...
                     alpha_acf=0, alpha_leverage=0, alpha_emd=1.0):
    """
    Single critic update in JAX, composite WGAN-GP
    Args:
        disc_params: Critic parameters
...
class quantum_GAN(object):
    ...
    def __init__(
        ...
        if save == True and resume_from == False:
            ...
            self.path = path + '/{}'.format("b_" + str(bond_dim) + "_L_" + str(n_layers) 
                                            ...
                                            + "_" + str(n_qubits) + exrtra_name + "Critic")
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            if self.save and resume_from == True:
